...
Charactor::Charactor(const std::string& name, std::size_t id, int maxHp, int attack, int defense, int speed, AttributeType type, bool playable)
	: name_(name), id_(id), maxHp_(maxHp), hp_(maxHp), attack_(attack), defense_(defense), speed_(speed), type_(type), playable_(playable) {
}
